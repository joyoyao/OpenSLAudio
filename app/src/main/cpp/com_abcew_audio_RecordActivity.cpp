...
#include <stdint.h>
#include "com_abcew_audio_RecordActivity.h"
#include "Log.h"
 ...
 */
    JNIEXPORT void JNICALL Java_com_abcew_audio_RecordActivity_createAudioRecord
        ...
        SLInterfaceID iids[NUM_EXPLICIT_INTERFACES_FOR_RECORDER] = {SL_IID_ANDROIDSIMPLEBUFFERQUEUE,
                                                                    SL_IID_ANDROIDCONFIGURATION};
        SLboolean required[NUM_EXPLICIT_INTERFACES_FOR_RECORDER] = {SL_BOOLEAN_TRUE,
                                                                    SL_BOOLEAN_TRUE};

        /* Create the audio recorder */
        result = (*engineEngine)->CreateAudioRecorder(engineEngine, &recorderObject, &recSource,
                                                      &dataSink,
                                                      NUM_EXPLICIT_INTERFACES_FOR_RECORDER, iids,
                                                      required);
        assert(SL_RESULT_SUCCESS == result);
        ...
        result = (*recBuffQueueItf)->RegisterCallback(recBuffQueueItf, recBufferQueueCallback,
                                                      &cntxt);
        assert(SL_RESULT_SUCCESS == result);
        ...
        for (int i = 0; i < NB_BUFFERS_IN_QUEUE; i++) {
            ALOGD("%d ", i);
            result = (*recBuffQueueItf)->Enqueue(recBuffQueueItf, cntxt.pData,
